package com.co.technicaltest.account_service.infrastructure.adapter.output.messaging.producer;

import com.co.technicaltest.account_service.domain.exception.KafkaProducerException;
...
public class KafkaProducerAdapter<K extends Serializable, V> implements KafkaProducerPort<K, V> {
    ...
    public void send(String topicName, K key, V message, BiConsumer<SendResult<K, V>, Throwable> callback) {
        ...
        try {
            ...
            log.error("Error en kafka al pruducir mensaje con key: {}, mensaje: {} y exception: {}", key, message,
                    e.getMessage());
